...
class Constants(BaseConstants):
    name_in_url = 'r_MPL_elicit'
    players_per_group = None
    num_rounds = 1
    Stage_Title = "Part 1 (continued): Applying the Switch-Point Procedure"
    num_sliders = 5
    fancy_piz_val_fields = [f'fancy_piz_val{i}' for i in range(1, num_sliders)]
    cheap_piz_val_fields = [f'cheap_piz_val{i}' for i in range(1, num_sliders)]
    fancy_tac_val_fields = [f'fancy_tac_val{i}' for i in range(1, num_sliders)]
    cheap_tac_val_fields = [f'cheap_tac_val{i}' for i in range(1, num_sliders)]
    val_fields = fancy_piz_val_fields + cheap_piz_val_fields + fancy_tac_val_fields + cheap_tac_val_fields
...
def make_slider_field(label):
    return models.IntegerField(
        choices=[10 * i for i in range(0, 11)],
        label=label,
    )
...
# Generate slider fields for all goods
for val_field in Constants.val_fields:
    setattr(Player, val_field, make_slider_field(""))
...
class SliderPage(Page):
    form_model = "player"
    form_fields = Constants.fancy_piz_val_fields
    ...
    def vars_for_template(player):
        ...
        qlist = []
        for q in range(1, 5):
            qdict = {'number': Constants.fancy_piz_val_fields[q - 1],  # Question number / name
                     'payment_amount': q + 1,  # Amount to pay
                     'payment_num': str(q)}  # Question number
            # Really just need to add the field for each value as a string, and good to go?
            qlist.append(qdict)
        rval['question_list'] = qlist
        rval['Task_name'] = 'Fancy Pizza'
        return rval


class ElicitationPage(Page):
    form_model = "player"
    form_fields = Constants.val_fields


class InstructionPage(Page):
    form_model = 'player'
    form_fields = ['Fancy_Pizza_Value', 'Cheap_Pizza_Value', 'Fancy_Taco_Value',
                   'Cheap_Taco_Value'] + Constants.val_fields

    @staticmethod
    def vars_for_template(player):
        rval = {}
        fpqlist, cpqlist, ftqlist, ctqlist = [], [], [], []
        for q in range(1, Constants.num_sliders):
            fpqdict = {'number': Constants.fancy_piz_val_fields[q - 1],  # Question number / name
                       'payment_amount': q + 1,  # Amount to pay
                       'payment_num': str(q)}  # Question number
            cpqdict = {'number': Constants.cheap_piz_val_fields[q - 1],  # Question number / name
                       'payment_amount': q + 1,  # Amount to pay
                       'payment_num': str(q)}  # Question number
            ftqdict = {'number': Constants.fancy_tac_val_fields[q - 1],  # Question number / name
                       'payment_amount': q + 1,  # Amount to pay
                       'payment_num': str(q)}  # Question number
            ctqdict = {'number': Constants.cheap_tac_val_fields[q - 1],  # Question number / name
                       'payment_amount': q + 1,  # Amount to pay
                       'payment_num': str(q)}  # Question number

            fpqlist.append(fpqdict)
            cpqlist.append(cpqdict)
            ftqlist.append(ftqdict)
            ctqlist.append(ctqdict)

        rval['fp_question_list'] = fpqlist
        rval['cp_question_list'] = cpqlist
        rval['ft_question_list'] = ftqlist
        rval['ct_question_list'] = ctqlist
